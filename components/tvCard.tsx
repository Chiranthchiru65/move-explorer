...
import fallBackImg from "../assets/fallback.png"; // Adjust path as needed
...
const TVCard = memo(function TVCard({ tvShow }: { tvShow: TVShow }) {
  const [imageError, setImageError] = useState(false);

  const memoizedData = useMemo(
    () => ({
      releaseYear: tvShow.first_air_date?.split("-")[0] || "N/A",
      formattedRating: tvShow.vote_average.toFixed(1),
      imageUrl: tvShow.poster_path
        ? `https://image.tmdb.org/t/p/w500${tvShow.poster_path}`
        : null,
      detailsUrl: `/dashboard/tv-details/${tvShow.id}`,
    }),
    [tvShow.first_air_date, tvShow.vote_average, tvShow.poster_path, tvShow.id]
  );
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <h3 className="font-semibold text-sm truncate">{tvShow.name}</h3>
